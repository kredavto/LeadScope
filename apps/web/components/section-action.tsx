...
export function SectionAction({ sectionKey, label }: { sectionKey: string; label: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      else if (sectionKey === "crawls") { const source = workspace.sources.find((item) => item.id === values.sourceId); const allowed = source?.status === "APPROVED" && source?.robots !== "DENIED"; if (!allowed) throw new Error("Сканирование заблокировано: источник должен иметь статус APPROVED и разрешённый robots."); id = record("crawls", { sourceId: values.sourceId, url: values.url, pages: 14, changed: 4, duration: "00:37", status: "COMPLETED", startedAt: "только что" }, "CRAWL_COMPLETE"); }
