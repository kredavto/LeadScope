...
- Рабочее пространство в браузере: создание, редактирование, статусы, удаление, поиск, уведомления и журналирование действий.
- AI Opportunity Agent на OpenAI `gpt-5.4-mini`: кнопка «Найти возможность» анализирует агрегированные предложения, сигналы и компании, применяет compliance-ограничения и сохраняет результат в Opportunity Board.
- Действующие workflows для источников и сканирований, компаний и сигналов, first-party лидов, suppression, DSR, retention, политик и безопасного CSV-экспорта.
- Состояние демо-workspace сохраняется локально в браузере без хранения открытых контактных данных; доступен сброс через очистку данных сайта.
...
демонстрационном режиме. AI-агент на Vercel использует защищённую OIDC-авторизацию
AI Gateway; ключи не попадают в клиентский код. Полный контур с FastAPI, PostgreSQL,
Redis и Celery запускается через Docker Compose. Для отдельного production-backend задайте
